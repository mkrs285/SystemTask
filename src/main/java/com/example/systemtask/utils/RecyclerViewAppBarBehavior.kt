...
class RecyclerViewAppBarBehavior(context: Context, attrs: AttributeSet) :
        ...
        if (target is RecyclerView) {
            if (scrollListenerMap[target] == null) {
                val recyclerViewScrollListener =
                    RecyclerViewScrollListener(coordinatorLayout, child, this)
    ...
    private class RecyclerViewScrollListener(
        coordinatorLayout: CoordinatorLayout,
        child: AppBarLayout,
        barBehavior: RecyclerViewAppBarBehavior
        ...
        private val coordinatorLayoutRef: WeakReference<CoordinatorLayout> =
            WeakReference(coordinatorLayout)
        private val childRef: WeakReference<AppBarLayout> = WeakReference(child)
        private val behaviorWeakReference: WeakReference<RecyclerViewAppBarBehavior> =
            WeakReference(barBehavior)
        ...
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            if (scrolledY <= 0 && !dragging && childRef.get() != null && coordinatorLayoutRef.get() != null && behaviorWeakReference.get() != null) {
                behaviorWeakReference.get()!!.onNestedFling(
